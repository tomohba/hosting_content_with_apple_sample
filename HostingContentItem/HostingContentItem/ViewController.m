...
- (void)viewDidLoad {
	...
	// すてにプロダクトを購入済みか判定する
	NSData *map = [[NSUserDefaults standardUserDefaults] objectForKey:PRODUCTID];
	if (map != nil) {
		// 購入済み、金額表示を購入済みに
		[self.productPrice setText:@"購入済み"];
        
		// 地図を表示する
		NSString *file = [[NSUserDefaults standardUserDefaults] objectForKey:PRODUCTID];
		if (file != nil) {
			NSData *map = [[NSFileManager alloc] contentsAtPath:file];
			if (map != nil) {
				self.mapImage.image = [[UIImage alloc] initWithData:map];
			}
		}
